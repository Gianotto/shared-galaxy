...
[ -f "$GAME/spacehaven.jar" ] || {
    echo "spacehaven.jar not found under $GAME" >&2
...
docker run --rm --user "$(id -u):$(id -g)" \
    -v "$HERE:/mod" -v "$AJ:/aj:ro" -v "$GAME:/game:ro" \
    eclipse-temurin:21-jdk \
    java -cp /aj/aspectjtools.jar org.aspectj.tools.ajc.Main \
        -source 8 -target 8 -Xlint:ignore \
        -bootclasspath /game/jre/lib/rt.jar \
        -classpath /aj/aspectjrt.jar:/game/spacehaven.jar \
        -d /mod/build/classes \
